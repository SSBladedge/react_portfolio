  ...
  return (
    <div className = "flex justify-between items-center w-full h-20 px-4 bg-black text-white fixed">
      ...
      <ul className='hidden md:flex'>
        ...
        {links.map(({ id, link}) =>(
          <li className='px-4 cursor-pointer capitalize font-medium hover:scale-105 text-gray-500 duration-200' key={id}>
              {link}
            </li>
        ))}
      ...
      <ul className= "flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-500">
          ...
          {links.map(({ id, link}) =>(
          <li className='px-4 cusor-pointer capitalize py-6 text-4xl' key={id}>
              {link}
            </li>
        ))}
      </ul>
